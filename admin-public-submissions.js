...
(function(){
  ...
  if(document.getElementById('publicSubmissionsButton')) return;
  ...
  button.id='publicSubmissionsButton';
  button.innerHTML='👥 Visitor Posts <span id="publicSubmissionsCount">0</span>';
  document.body.appendChild(button);

  const overlay=document.createElement('div');
  overlay.id='publicSubmissionsOverlay';
  overlay.innerHTML=`<div class="ps-panel"><div class="ps-head"><div><h2>Visitor Product Submissions</h2><p>Review public seller listings before they appear on Akeem Store.</p></div><button class="ps-close" aria-label="Close">×</button></div><div class="ps-body"><div id="publicSubmissionsMessage"></div><div id="publicSubmissionsList"></div></div></div>`;
  document.body.appendChild(overlay);

  const list=document.getElementById('publicSubmissionsList');
  const msg=document.getElementById('publicSubmissionsMessage');
  const count=document.getElementById('publicSubmissionsCount');
  const close=overlay.querySelector('.ps-close');
  button.onclick=()=>{overlay.classList.add('open');load()};
  ...
  async function load(){
    ...
    try{
      ...
      const rows=[];snap.forEach(d=>rows.push({id:d.id,...d.data()}));rows.sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));
      ...
      list.querySelectorAll('[data-approve]').forEach(b=>b.onclick=()=>approve(b.dataset.approve));
      list.querySelectorAll('[data-reject]').forEach(b=>b.onclick=()=>reject(b.dataset.reject));
  ...
  function render(p){
    ...
    const c=p.sellerContact||{};
    const contacts=[c.phone,c.email,c.other].filter(Boolean).map(esc).join(' • ')||'No contact supplied';
