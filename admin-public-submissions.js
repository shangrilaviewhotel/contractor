...
import { collection, getDocs, getDoc, updateDoc, doc, query, where, writeBatch } from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';
...
(function(){
  ...
  style.textContent=`
    ...
    .ps-body{padding:20px}.ps-loading,.ps-empty{padding:35px;text-align:center;color:#6b7280}.ps-card{border:1px solid #e7e5f0;border-radius:14px;margin-bottom:14px;overflow:hidden;background:#fff}.ps-top{display:flex;gap:16px;padding:16px}.ps-thumb{width:170px;height:125px;flex:0 0 170px;display:grid;grid-template-columns:1fr 1fr;gap:4px;background:#f3f4f6;border-radius:10px;overflow:hidden}.ps-thumb img,.ps-thumb video{width:100%;height:100%;object-fit:cover}.ps-info{flex:1;min-width:0}.ps-info h3{margin:0 0 5px;font:800 16px Sora,Montserrat,sans-serif}.ps-price{font-size:18px;font-weight:800;color:#059669}.ps-compare{font-size:11px;color:#98a2b3;text-decoration:line-through;margin-left:7px}.ps-meta{display:flex;flex-wrap:wrap;gap:7px;margin-top:8px}.ps-chip{background:#f5f6ff;border:1px solid #ecebf7;border-radius:20px;padding:4px 8px;font-size:10.5px;color:#5f6170}.ps-desc{font-size:12px;color:#5f6170;margin:9px 0 0;line-height:1.5}.ps-tags{margin-top:8px;font-size:10.5px;color:#667085}.ps-contact{margin-top:10px;padding:10px;border-radius:9px;background:#fff7ed;border:1px solid #fed7aa;color:#7c2d12;font-size:12px}.ps-actions{display:flex;gap:8px;padding:12px 16px;border-top:1px solid #ecebf7;background:#fbfaff}.ps-actions button{border:0;border-radius:8px;padding:9px 13px;font:700 12px Montserrat;cursor:pointer}.ps-approve{background:#16a765;color:#fff}.ps-reject{background:#ffecec;color:#c24141}.ps-actions button:disabled{opacity:.55;cursor:wait}.ps-error{padding:12px;background:#ffecec;color:#991b1b;border-radius:9px;margin-bottom:14px;font-size:12px}
  ...
  async function load(){
    ...
    try{
      ...
      list.querySelectorAll('[data-approve]').forEach(b=>b.onclick=()=>approve(b.dataset.approve,b));
      list.querySelectorAll('[data-reject]').forEach(b=>b.onclick=()=>reject(b.dataset.reject,b));
  ...
  function render(p){
    const media=(p.imageUrls||[]).slice(0,4).map(u=>isVideo(u)?`<video src="${esc(u)}" muted controls preload="metadata"></video>`:`<img src="${esc(u)}" alt="${esc(p.imageAlt||p.name||'Product')}">`).join('');
    const c=p.sellerContact||{};const contacts=[c.phone,c.email,c.other].filter(Boolean).map(esc).join(' • ')||'No contact supplied';
    const tags=Array.isArray(p.tags)&&p.tags.length?`<div class="ps-tags"><strong>Tags:</strong> ${p.tags.map(esc).join(', ')}</div>`:'';
    return `<article class="ps-card"><div class="ps-top"><div class="ps-thumb">${media||'<div style="grid-column:1/-1;display:grid;place-items:center;color:#98a2b3">No media</div>'}</div><div class="ps-info"><h3>${esc(p.name||'Untitled product')}</h3><div class="ps-price">${money(p.price)}${p.comparePrice?`<span class="ps-compare">${money(p.comparePrice)}</span>`:''}</div><div class="ps-meta"><span class="ps-chip">${esc(p.category||'Uncategorized')}</span>${p.brand?`<span class="ps-chip">${esc(p.brand)}</span>`:''}${p.location?`<span class="ps-chip">📍 ${esc(p.location)}</span>`:''}${p.condition?`<span class="ps-chip">${esc(p.condition)}</span>`:''}<span class="ps-chip">Stock: ${esc(p.stockQuantity??1)}</span></div><p class="ps-desc">${esc(p.description||'No description')}</p>${tags}<div class="ps-contact"><strong>Seller contact:</strong> ${contacts}</div></div></div><div class="ps-actions"><button class="ps-approve" data-approve="${esc(p.id)}">✓ Approve & Publish</button><button class="ps-reject" data-reject="${esc(p.id)}">Reject</button></div></article>`;
  }

  async function approve(id,buttonEl){
    if(!confirm('Approve this visitor listing and publish it to the public store?'))return;
    if(buttonEl){buttonEl.disabled=true;buttonEl.textContent='Publishing...'}
    try{
      const snap=await getDoc(doc(db,'pendingProductSubmissions',id));if(!snap.exists())throw new Error('Submission not found');
      const p={id:snap.id,...snap.data()};
      if(p.status!=='pending'){await load();return}
      ...
      const product={
        ...
        imageUrls:Array.isArray(p.imageUrls)?p.imageUrls:[],sellerContact:p.sellerContact||{},source:'public-seller',
        createdAt:p.createdAt||Date.now(),updatedAt:Date.now(),approvedAt:Date.now(),approvedBy:auth.currentUser?.email||auth.currentUser?.uid||'admin'
      };
      const batch=writeBatch(db);
      batch.set(productRef,product);
      batch.update(doc(db,'pendingProductSubmissions',id),{status:'approved',reviewStatus:'approved',approvedAt:Date.now(),approvedBy:auth.currentUser?.email||auth.currentUser?.uid||'admin',published:true,publishedProductId:productRef.id,updatedAt:Date.now()});
      await batch.commit();
      await load();
    }catch(e){console.error(e);msg.innerHTML=`<div class="ps-error">Approval failed: ${esc(e.message)}</div>`;if(buttonEl){buttonEl.disabled=false;buttonEl.textContent='✓ Approve & Publish'}}
  }

  async function reject(id,buttonEl){
    const reason=prompt('Optional reason for rejecting this listing:','');if(reason===null)return;
    if(buttonEl){buttonEl.disabled=true;buttonEl.textContent='Rejecting...'}
    try{await updateDoc(doc(db,'pendingProductSubmissions',id),{status:'rejected',reviewStatus:'rejected',rejectionReason:reason.trim(),rejectedAt:Date.now(),rejectedBy:auth.currentUser?.email||auth.currentUser?.uid||'admin',updatedAt:Date.now()});await load();}
    catch(e){console.error(e);msg.innerHTML=`<div class="ps-error">Rejection failed: ${esc(e.message)}</div>`;if(buttonEl){buttonEl.disabled=false;buttonEl.textContent='Reject'}}
  }
})();
